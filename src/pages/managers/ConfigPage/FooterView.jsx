      ...
      <div className="3xl:max-w-full 3xl:w-full md:max-w-6xl mx-auto 3xl:px-20">
        <div className="grid grid-cols-1 gap-8 mb-8 md:grid-cols-2 lg::grid-cols-3 xl:grid-cols-3">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                <MapPin
                  className="flex-shrink-0 mt-1 text-red-500 xs:text-sm md:text-xl"
                  size={20}
                />
              ...
              <div className="flex items-center gap-3">
                <Phone
                  ...
                  size={20}
                />
                <p className="xs:text-sm md:text-md 3xl:text-xl">
                  {currentData.phone}
                </p>
              </div>
              <div className="flex items-center gap-3">
              ...
              {/* ✅ Bộ Công Thương hiển thị khi bật enable */}
              {boCongThuong?.enable && boCongThuong?.url && (
                <div className="flex items-center gap-3">
                  <a
                    href={boCongThuong.url}
                    target="_blank"
          ...
          <div className="space-y-8">
            <div>
              <h3 className="mb-4 text-xl font-semibold 3xl:text-3xl">
                {t("footer.aboutUs")}
              </h3>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-x-8">
                <ul className="space-y-3">
                  {privacy_statement
                    .slice(0, Math.ceil(privacy_statement.length / 2))
                    .map((service, index) => (
                      <li
                        key={index}
                        className="transition-all duration-200 hover:text-green-400 3xl:text-xl"
                      >
                ...
                <ul className="space-y-3">
                  {privacy_statement
                    .slice(Math.ceil(privacy_statement.length / 2))
                    .map((service, index) => (
                      <li
                        key={index + 100}
                        className="transition-all duration-200 hover:text-green-400 xs:text-sm md:text-md 3xl:text-xl"
                      >
                        <Link to={service.link}>{service.title}</Link>
                      </li>
                    ))}
        ...
        <div className="flex flex-col items-center justify-between pt-6 border-t border-gray-700 md:flex-row">
          ...
          <div className="flex gap-3">
            {socials
              ...
              .map((soc, idx) => {
                const key = normalizeKey(soc.title);
