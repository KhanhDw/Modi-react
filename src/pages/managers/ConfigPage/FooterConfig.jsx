...
export default function FooterConfigMultiLang() {
  ...
  const fetchFooter = async () => {
    try {
      ...
      if (logoData.length > 0) {
        ...
        setPreview(
          logoData[0].image_url ? `${API_BASE_URL}${logoData[0].image_url}` : ""
        );
      }
  ...
  if (loading)
    return (
      <p className="text-center text-xs sm:text-sm text-gray-700 admin-dark:text-gray-300 py-6">
        ⏳ Đang tải...
      </p>
    );
      ...
      <FooterView
        data={{
          [activeLang]: {
            logo: preview,
            name_company:
              companyInfo.find((f) => f.position === 2)?.description?.[
                activeLang
              ] || "",
            content_about_us:
              companyInfo.find((f) => f.position === 6)?.description?.[
                activeLang
              ] || "",
            address_company:
              companyInfo.find((f) => f.position === 3)?.description?.[
                activeLang
              ] || "",
            phone:
              companyInfo.find((f) => f.position === 4)?.description?.[
                activeLang
              ] || "",
            email:
              companyInfo.find((f) => f.position === 5)?.description?.[
                activeLang
              ] || "",
          },
        }}
          ...
          <div className="flex items-center justify-center gap-2 xs:gap-3 ">
            <div className="border border-gray-300 flex flex-col z-2 rounded-lg p-3 xs:w-full xs:flex-row items-center justify-end gap-2">
              ...
              <Switch
                ...
                onClick={() =>
                  setActiveLang((pre) => (pre === "vi" ? "en" : "vi"))
                }
              />
            </div>
          </div>
                ...
                >
                  ...
                  {isLongText ? (
                    <textarea
                      ...
                      value={value}
                      onChange={(e) =>
                        handleChangeInfo(info.id, "description", e.target.value)
                      }
                      rows={8}
          ...
          <div className="p-3 sm:p-4 rounded-lg sm:rounded-xl bg-white admin-dark:bg-gray-800 shadow space-y-3 sm:space-y-4 border border-gray-300 admin-dark:border-gray-600 transition">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-3">
              ...
              <p className="text-xs sm:text-sm text-gray-500 admin-dark:text-gray-400">
                Không có tiêu đề dịch vụ đó sẽ không hiển thị
              </p>
            </div>
            {services.map((s) => (
              <div
              ...
              >
                <div className="flex flex-col gap-1 sm:gap-2">
                  <p className="text-xs sm:text-sm text-gray-500 admin-dark:text-gray-400 whitespace-nowrap">
                    Tiêu đề
                  </p>
                  <input
                    autoComplete="off"
                ...
                <div className="flex flex-col gap-1 sm:gap-2">
                  ...
                  {s.description?.[activeLang]?.length > 70 ? (
                    <textarea
                      ...
                      onChange={(e) =>
                        updateService(s.id, "description", e.target.value, true)
                      }
                      rows={4}
                    />
                  ) : (
              ...
              >
                ...
                {s.description?.[activeLang]?.length > 70 ? (
                  <textarea
                    ...
                    value={s.description?.[activeLang] || ""}
                    onChange={(e) =>
                      updateSocial(s.id, "description", e.target.value, true)
                    }
                    rows={4}
                  />
                ) : (
                ...
                >
                  <div className="flex flex-col gap-1 sm:gap-2">
                    ...
                    <input
                      ...
                      className={`w-full p-2 sm:p-3 rounded border border-gray-300 admin-dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-300 transition text-xs sm:text-sm ${
                        !enableBoCongThuong
                          ? "text-gray-400 admin-dark:text-gray-600"
                          : "text-gray-800 admin-dark:text-gray-200"
                      }`}
                      placeholder="URL"
