...
export default function ServiceSelectionModal({
  ...
  const fetchDataService = async () => {
    try {
      ...
      // Kiểm tra currentStage là object hay chỉ là id
      const currentStageId =
        typeof currentStage === "object" ? currentStage.id : currentStage;

      // Danh sách service đã chọn trong stage hiện tại
      const preSelected = serviceResult.filter((s) =>
        serviceStageResult.some(
          (st) =>
            st.service_id === s.id &&
            String(st.stage_id) === String(currentStageId)
        )
      );

      // Đánh dấu service nào đã nằm ở stage khác (disable)
      const withOtherStageFlag = serviceResult.map((s) => ({
        ...s,
        inOtherStage: serviceStageResult.some(
          (ss) =>
            ss.service_id === s.id &&
            String(ss.stage_id) !== String(currentStageId)
        ),
      }));
              ...
              .map((service) => {
                ...
                return (
                  <div
                    key={service.id}
                    onClick={() =>
                      !isDisabled && handleSelectService(service.id)
                    }
                    className={cn(
        ...
        <DialogFooter className="pt-4 border-t admin-dark:border-gray-700">
          <div className="flex items-center justify-between w-full">
            <div>
              <Badge
                variant="outline"
                className="text-gray-700 admin-dark:text-primary/70 border border-primary/70"
              >
            ...
            <div className="flex gap-2">
              <DialogClose>
                <span className="rounded-lg px-3 py-2 text-sm sm:text-base font-semibold bg-gray-200 hover:bg-gray-300 admin-dark:bg-gray-800 admin-dark:hover:bg-gray-700 cursor-pointer">
                  Đóng
                </span>
              </DialogClose>
              <Button
