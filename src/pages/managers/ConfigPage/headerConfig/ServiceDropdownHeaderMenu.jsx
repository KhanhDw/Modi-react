...
export default function ServiceDropdownHeaderMenu({ lang = "vi" }) {
  ...
  const fetchAllServicesMenu = async () => {
    try {
      ...
      const requests = sectionsRes.data
        ...
        .map((section) =>
          fetch(
            `${API_BASE_URL}/api/section-items/type/${section.type}?slug=header`
          )
            .then((r) => r.json())
            .then((items) => ({
              id: section.id,
  ...
  const fetchChildren = async (category) => {
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/section-items/type/${category.type}?slug=header`
      );
      ...
      setMenuData((prev) =>
        prev.map((cat) =>
          cat.id === category.id ? { ...cat, children: updatedChildren } : cat
        )
      );

      setSelectedCategory((prev) =>
        prev && prev.id === category.id
          ? { ...prev, children: updatedChildren }
          : prev
      );
    } catch (err) {
      console.error(err);
  ...
  const addCategory = async (nameEn, nameVi, listIdServices) => {
    try {
      ...
      if (!res.ok || !data.success)
        throw new Error(data.error || "Không thể thêm");
  ...
  const addChild = async (nameEn, nameVi, slug) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok || !data.success)
        throw new Error(data.error || "Không thể thêm mục con");
      ...
      setMenuData((prev) =>
        prev.map((cat) => (cat.id === selectedCategory.id ? updatedCat : cat))
      );
  ...
  const updateCategory = async (catId, nameEn, nameVi, listIdServices) => {
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/service-header-config/${catId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
  ...
  const updateChild = async (childId, nameEn, nameVi, slug) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok || !data.success)
        throw new Error(data.error || "Không thể cập nhật mục con");
  ...
  const deleteChild = async (child) => {
    try {
      ...
      const data = await res.json();
      if (!res.ok || !data.success)
        throw new Error(data.error || "Không thể xóa mục con");
  ...
  async function updateSectionItemPositionsChild(data) {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/section-items/update/bulk-update-positions`,
        {
          method: "PUT",
          headers: {
  ...
  const openAddChild = (parentCategory) => {
    ...
    const servicesForParent = groupServices
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
  ...
  const openEditCategory = (cat) => {
    const groupServices = String(
      cat.name?.groupServices || cat.name?.groupServices || ""
    )
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
  ...
  const setDialogListIdServices = (valOrUpdater) => {
    setDialog((prev) => {
      const current = Array.isArray(prev.listIdServices)
        ...
        ? prev.listIdServices
          ? prev.listIdServices.split(",")
          : []
        : [];
      const newList =
        typeof valOrUpdater === "function"
          ? valOrUpdater(current)
          : valOrUpdater;
  ...
  const setDialogValueEn = (val) => {
    setDialog((prev) =>
      prev.valueEn === val ? prev : { ...prev, valueEn: val }
    );
  };

  const setDialogValueVi = (val) => {
    setDialog((prev) =>
      prev.valueVi === val ? prev : { ...prev, valueVi: val }
    );
  };

  const setDialogValueSlug = (val) => {
    setDialog((prev) =>
      prev.valueSlug === val ? prev : { ...prev, valueSlug: val }
    );
  };
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 min-h-[550px] h-fit">
      ...
      <CategoryList
        ...
        onDelete={(cat) =>
          setConfirmDialog({ open: true, type: "category", target: cat })
        }
      ...
      <ChildList
        ...
        onDelete={(child) =>
          setConfirmDialog({ open: true, type: "child", target: child })
        }
          ...
          <DialogForm
            ...
            usedServiceSlugs={
              selectedCategory?.children
                ?.map((child) => child.description?.en)
                .filter(Boolean) || []
            }
