...
function FileInput({ label, onChange }) {
  return (
    <div className="flex flex-col gap-2">
      <label className="font-semibold text-gray-700 admin-dark:text-gray-300 text-xs sm:text-sm">
        {label}
      </label>
...
export default function HeaderConfigLogo() {
  ...
  if (loading)
    return (
      <p className="text-center text-gray-700 admin-dark:text-gray-300 text-xs sm:text-sm py-6">
        ⏳ Đang tải...
      </p>
    );

  return (
    <div className="md:p-2 mx-auto flex flex-col items-center space-y-6 sm:space-y-8">
      {/* Header Preview */}
      <motion.div
        className="max-w-3xl w-full flex flex-col items-center justify-center bg-indigo-50 admin-dark:bg-gray-800 rounded-xl shadow-lg p-2 sm:p-6 md:p-8 gap-4 sm:gap-6"
      ...
      {/* Config Form */}
      <motion.div
        className="bg-white max-w-3xl w-full admin-dark:bg-gray-900 px-2 py-3 sm:p-4 rounded-xl shadow-md border border-gray-200 admin-dark:border-gray-700 space-y-4 md:space-y-6"
      ...
      {/* Config Form */}
      <motion.div
        className="w-full bg-white admin-dark:bg-gray-900 px-2 py-3 sm:px-4 rounded-xl sm:rounded-xl shadow-md border border-gray-200 admin-dark:border-gray-700 space-y-4 sm:space-y-6"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.7 }}
      >
        <div className="w-full flex flex-col space-y-2 md:space-y-4">
          <span className="font-semibold text-gray-700 admin-dark:text-gray-300 text-xs sm:text-sm">
