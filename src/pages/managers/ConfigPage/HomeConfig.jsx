...
export default function HomeConfigMultiLang() {
  ...
  const [activeSection, setActiveSection] = useState(
    () =>
      localStorage.getItem("activeHomeConfigSection") || sectionsConfig[0].key
  );
  const [previewBanner, setPreviewBanner] = useState({});
  const [toast, setToast] = useState(null);

  useEffect(() => {
    localStorage.setItem("activeHomeConfigSection", activeSection);
  }, [activeSection]);
          ...
          data.map(async (item) => {
            ...
            if (item.file instanceof File) {
              ...
              if (url) {
                setHomeData((prev) => {
                  const copy = { ...prev };
                  copy[activeLang][section] = copy[activeLang][section].map(
                    (x) =>
                      x.id === item.id
                        ? { ...x, banner: url, file: undefined }
                        : x
                  );
                  return copy;
                });
            ...
            <div className="flex justify-end">
              <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-gray-100 admin-dark:bg-gray-700 shadow-sm">
                ...
                <Switch
                  ...
                  onClick={() =>
                    setActiveLang((prev) => (prev === "vi" ? "en" : "vi"))
                  }
                />
              </div>
            </div>
