...
export default function ContactPage() {
    ...
    const baseColumns = [
      {
        key: "ho_ten",
        label: "Họ tên",
        className: "font-semibold admin-dark:text-gray-100 text-gray-900 overflow-hidden",
        render: (value) => (
      ...
      },
      {
        ...
        className: "overflow-hidden",
        render: (value) => (
          <span className="font-mono text-xs sm:text-sm admin-dark:text-gray-200 text-gray-700 truncate">{value}</span>
        ),
      },
      {
        key: "trang_thai",
        ...
        render: (value) => {
          ...
          return (
            <div className="text-xs sm:text-sm">
              <div className="font-medium admin-dark:text-gray-100 text-gray-900 truncate">{date.toLocaleDateString('vi-VN')}</div>
              <div className="admin-dark:text-gray-400 text-gray-500 text-xs truncate">{date.toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}</div>
            </div>
          );
        },
      },
  ...
  const handleStatusChange = (id, newStatus) => {
    fetch(`${import.meta.env.VITE_MAIN_BE_URL}/api/lienhe/${id}`, {
      ...
      .then((response) => response.json())
      .then(() => {
        setContacts((prev) => prev.map((c) => (c.id === id ? { ...c, trang_thai: newStatus } : c)));
        if (showDetail && showDetail.id === id) {
  ...
  const Pagination = () => {
    ...
    return (
      <div className="flex items-center justify-between border-t admin-dark:border-gray-700 border-gray-200 admin-dark:bg-gray-800 bg-white px-2 sm:px-4 py-3">
        <div className="flex flex-1 justify-between sm:hidden">
          <button
            onClick={() => setCurrentPage(Math.max(1, currentPage - 1))}
            disabled={currentPage === 1}
            className="relative inline-flex items-center rounded-md border admin-dark:border-gray-600 admin-dark:bg-gray-700 admin-dark:text-gray-300 admin-dark:hover:bg-gray-600 border-gray-300 bg-white px-3 py-2 text-xs sm:text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Trước
          </button>
          <button
            ...
            className="relative ml-2 sm:ml-3 inline-flex items-center rounded-md border admin-dark:border-gray-600 admin-dark:bg-gray-700 admin-dark:text-gray-300 admin-dark:hover:bg-gray-600 border-gray-300 bg-white px-3 py-2 text-xs sm:text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Sau
          </button>
        </div>
        <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
          <div className="text-xs sm:text-sm admin-dark:text-gray-300 text-gray-700">
            Hiển thị {startIndex + 1} - {Math.min(endIndex, filteredContacts.length)} / {filteredContacts.length}
          </div>
          <div>
            <nav className="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
              ...
              >
                ...
                <svg className="h-4 sm:h-5 w-4 sm:w-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
              ...
              {startPage > 1 && (
                <>
                  <button
                    ...
                    className="relative inline-flex items-center px-3 sm:px-4 py-2 text-xs sm:text-sm font-semibold admin-dark:text-gray-200 admin-dark:ring-gray-600 admin-dark:hover:bg-gray-700 text-gray-900 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 focus:z-20 focus:outline-offset-0"
                  >
                    1
                  </button>
                  {startPage > 2 && (
                    <span className="relative inline-flex items-center px-3 sm:px-4 py-2 text-xs sm:text-sm font-semibold admin-dark:text-gray-400 admin-dark:ring-gray-600 text-gray-700 ring-1 ring-inset ring-gray-300">
                      ...
                    </span>
                  )}
                </>
              ...
              <div className="flex items-center">
                ...
                <div className="ml-2 sm:ml-4 w-0 flex-1">
                  <dl>
                    ...
                    <dd className="text-lg sm:text-xl font-bold admin-dark:text-gray-100 text-gray-900">{total}</dd>
                  </dl>
                </div>
                {statusFilter === "all" && dateFilter === "all" && (
                  <div className="ml-2">
          ...
          >
            <div className="p-3 sm:p-4">
              <div className="flex items-center">
                <div className="flex-shrink-0">
                  <div
                    className={`w-8 sm:w-10 h-8 sm:h-10 rounded-lg flex items-center justify-center ${statusFilter === "pending" ? "bg-amber-600" : "bg-amber-500"}`}
                  >
                    <svg className="w-4 sm:w-5 h-4 sm:h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="ml-2 sm:ml-4 w-0 flex-1">
                  <dl>
                    ...
                    <dd className="text-lg sm:text-xl font-bold admin-dark:text-gray-100 text-gray-900">{pending}</dd>
                  </dl>
                </div>
                {statusFilter === "pending" && (
                  <div className="ml-2">
              ...
              <div className="flex items-center">
                ...
                <div className="ml-2 sm:ml-4 w-0 flex-1">
                  <dl>
                    ...
                    <dd className="text-lg sm:text-xl font-bold admin-dark:text-gray-100 text-gray-900">{responded}</dd>
                  </dl>
                </div>
                {statusFilter === "responded" && (
                  <div className="ml-2">
