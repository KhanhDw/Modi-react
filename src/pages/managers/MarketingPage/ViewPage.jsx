...
export const ShowContextBlog = ({ parsedContent }) => {
  ...
  if (typeof parsedContent === "string") {
    try {
      ...
      return (
        <p className="text-gray-800 admin-dark:text-gray-200">
          {parsedContent}
        </p>
      );
    }
...
export default function ViewPage() {
  ...
  const fetchPostData = async () => {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_MAIN_BE_URL}${langPath}/api/marketing/id/${id}`
      );
      if (!res.ok) {
      ...
      setError(
        "Không tìm thấy bài viết hoặc bạn chưa cập nhật ngôn ngữ này cho bài viết"
      );
    } finally {
      setLoading(false);
    }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen admin-dark:bg-gray-900">
        <div className="text-base sm:text-lg font-semibold text-gray-700 admin-dark:text-gray-300">
          Đang tải bài viết...
        </div>
      </div>
    );
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen admin-dark:bg-gray-900 px-4">
        <p className="text-base sm:text-lg text-center text-red-600 admin-dark:text-red-400 mb-4">
          {error}
        </p>
        <button
          onClick={() => navigate(-1)}
  ...
  if (!post) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen admin-dark:bg-gray-900 px-4">
        <p className="text-xl text-gray-500 admin-dark:text-gray-400 mb-4">
          Ngôn ngữ cho tiếng Anh của bài viết chưa có.
        </p>
        <button
          onClick={() => navigate(-1)}
    ...
    <div className="relative transition-all admin-dark:bg-gray-800 px-4 py-2 rounded-lg duration-300 min-h-screen">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-2">
          ...
          <button
            onClick={() => setLang("en")}
            className={`px-4 py-2 rounded-md cursor-pointer font-bold transition-colors duration-200 ${
              lang === "en"
        ...
        <article className="mt-4">
          ...
          {post.tags && post.tags.length > 0 && (
            <footer className="pt-6 border-t border-gray-200 admin-dark:border-gray-700 mt-8">
              <h3 className="text-xs sm:text-sm md:text-base font-semibold text-gray-800 admin-dark:text-white mb-3">
                Tags:
              </h3>
