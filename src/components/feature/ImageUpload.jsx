  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (selectedFiles.length === 1) {
        ...
        if (fileNameImage) {
          deleteImage(fileNameImage.split("/").pop());
        }
        ...
        {showPreview && previewImages.length > 0 && (
          <div className="mb-4">
            <h3 className="font-medium mb-2">
              Preview ({previewImages.length} files):
            </h3>
            <div
              className={`grid gap-4 grid-cols-${gridColumns.sm} md:grid-cols-${gridColumns.md} lg:grid-cols-${gridColumns.lg}`}
            >
      ...
      {isUploadNewImage === true && showUploadedList && (
        <div className={`${currentTheme.container} p-6`}>
          ...
          {uploadedImages.length === 0 ? (
            ...
            <div
              className={`grid gap-6 grid-cols-1 md:grid-cols-${gridColumns.md} lg:grid-cols-${gridColumns.lg}`}
            >
