...
function AnimationHeader({ ActiveSideBarHeader }) {
    ...
    const [animatedBackgroundColor, setAnimatedBackgroundColor] = useState("rgba(25, 27, 31, 0.95)");
    ...
    useEffect(() => {
        const color = "rgba(25, 27, 31, 0.95)"; // Nền mờ cố định, loại bỏ điều kiện trong suốt
    ...
    return (
        <>
            <motion.div
                ...
                initial={{ backgroundColor: "rgba(25, 27, 31, 0.95)" }} // Bắt đầu với nền mờ
                animate={{
                    backgroundColor: animatedBackgroundColor,
                    height: location.pathname === "/"
                        ? (scrolled
                            ? (isMobile ? 80 : (isTablet ? 80 : (isDesktop ? 80 : 80)))
                            : (isMobile ? 80 : (isTablet ? 80 : (isDesktop ? 80 : 80)))) // Giữ chiều cao cố định 80px
                        : (isMobile ? 80 : (isTablet ? 80 : (isDesktop ? 80 : 80))),
