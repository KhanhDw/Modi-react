...
function ExcelDataUploader({
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const expectedHeaders = Object.keys(columnHeaders);
        ...
        if (missingHeaders.length > 0) {
          setError(
            `Tệp không hợp lệ. Thiếu các cột bắt buộc: ${missingHeaders
              .map((h) => columnHeaders[h])
              .join(", ")}. Vui lòng tải file mẫu để xem định dạng đúng.`
          );
          handleClearData();
          return;
        }
        ...
        setExcelData(null);
        setError(
          "Lỗi khi đọc file. Vui lòng kiểm tra lại định dạng file hoặc mã hóa UTF-8."
        );
        handleClearData();
      }
    };
  ...
  const handleDownloadTemplate = () => {
    const templateData = [
      ["name", "phone", "email", "address", "cccd", "number_bank", "name_bank"],
      [
        "Nguyễn Văn A",
        "0912345678",
    ...
    const blob = new Blob([bom + csvOutput], {
          ...
          <div className="mt-8 border border-gray-200 admin-dark:border-gray-600 rounded-lg shadow-sm overflow-hidden">
            <div className="p-4 flex flex-col gap-4 md:flex-row items-center justify-between bg-gray-50 admin-dark:bg-gray-800 border-b border-gray-200 admin-dark:border-gray-600">
              ...
              <p className="text-xs text-gray-500 admin-dark:text-gray-400">
                Cột "Ngân hàng" vui lòng chỉ nhật 3 ký tự viết tắt của ngân hàng
                như VietComBank = VCB
              </p>
              <button
              ...
              >
                <span className="text-sm sm:text-base">Hủy chọn File</span>
              </button>
            </div>
                  ...
                  {excelData.map((row, rowIndex) => (
                    ...
                    >
                      {Object.keys(columnHeaders).map((header) => (
                        <td
                          key={header}
                          className="px-3 py-3 text-sm text-gray-900 admin-dark:text-gray-100"
                        >
                          {row[header]}
                        </td>
                      ))}
