...
function ConfigCustomerVIP({ setOpenConfigCustomerVIP, onSaveSuccess }) {
  ...
  if (loading)
    return <p className="text-center p-4">⏳ Đang tải cấu hình VIP...</p>;

  return (
    <div
      onClick={() => setOpenConfigCustomerVIP(false)}
      className="fixed inset-0 bg-gray-300 bg-opacity-70 admin-dark:bg-gray-950/80 flex items-center justify-center z-50 p-4"
    >
      <div
      ...
      >
        ...
        <div className="p-3 sm:p-5 border-t border-gray-200 flex justify-center sm:justify-end space-x-3 bg-gray-50 rounded-b-xl admin-dark:border-gray-700 admin-dark:bg-gray-800">
          ...
          <button
            onClick={handleSave}
            className={`px-5 py-2 cursor-pointer text-sm font-medium text-white rounded-lg shadow-md ${
              isSaving
