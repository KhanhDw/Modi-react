...
export default function ReaderDetailService() {
    ...
    const service = initDataService.find((s) => s.translation?.slug === slug);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 items-start animate-fade-in-up">
                        ...
                        <div className="absolute bottom-6 left-6 text-white z-10">
                            ...
                            <div className="flex flex-wrap gap-2 mt-2">
                                {service.totalLanguages.map((lang, index) => (
                                    <Badge key={index} className={`flex items-center gap-1 font-semibold ${lang === "vi" ? "bg-red-500 hover:bg-red-600" : "bg-blue-500 hover:bg-blue-600"} transition-colors`}>
                                        <Globe size={14} />
                                        {lang.toUpperCase()}
                                    </Badge>
                                ))}
                                <Badge className={`flex items-center gap-1 font-semibold ${service.status === "Active" ? "bg-green-500 hover:bg-green-600" : "bg-gray-500 hover:bg-gray-600"} transition-colors`}>
                                    {service.status === "Active" ? (<><CheckCircle size={14} />Hoạt động</>) : (<><XCircle size={14} />Không hoạt động</>)}
                                </Badge>
                            </div>
                        </div>
                    </div>
                ...
                <div className="space-y-8 animate-fade-in-up delay-200">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <Card className="rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-[1.03] admin-dark:bg-gray-800">
                            <CardContent className="flex items-center gap-4 p-5">
                                ...
                                <div>
                                    <p className="text-sm font-medium text-gray-500 admin-dark:text-gray-400">Giá khởi điểm</p>
                                    <p className="text-xl font-bold text-green-600 admin-dark:text-green-400">
                                        {service.floor_price ? `₫${Number(service.floor_price).toLocaleString()}` : "Chưa có"}
                                    </p>
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <Card className="rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-[1.03] admin-dark:bg-gray-800">
                            <CardContent className="flex items-center gap-4 p-5">
                                ...
                                <div>
                                    <p className="text-sm font-medium text-gray-500 admin-dark:text-gray-400">Số lần đặt</p>
                    ...
                    <div className="space-y-4 pt-4 border-t border-gray-200 admin-dark:border-gray-700">
                        <h3 className="text-2xl font-bold text-gray-900 admin-dark:text-white">Mô tả dịch vụ</h3>
                    ...
                    {features.length > 0 && (
                        <div className="space-y-4 pt-4 border-t border-gray-200 admin-dark:border-gray-700">
                            <h3 className="text-2xl font-bold text-gray-900 admin-dark:text-white">Điểm nổi bật</h3>
                    ...
                    {details.length > 0 && (
                        <div className="space-y-4 pt-4 border-t border-gray-200 admin-dark:border-gray-700">
                            <h3 className="text-2xl font-bold text-gray-900 admin-dark:text-white">Chi tiết dịch vụ</h3>
