  ...
  useEffect(() => {
    const fetchTopSamples = async () => {
      try {
        ...
        }/api/web-samples/most-viewed?limit=5&lang=vi`;
  ...
  if (!topSamples || topSamples.length === 0) {
    return (
      <Card className="bg-white text-gray-900 border border-gray-200 admin-dark:border-gray-700 admin-dark:bg-gray-900 admin-dark:text-gray-100">
        <CardHeader>
          <CardTitle className="text-base sm:text-[18px] md:text-xl lg:text-xl font-bold text-gray-900 admin-dark:text-gray-300">
            Sản phẩm được xem nhiều nhất
          </CardTitle>
          <CardDescription>
  ...
  return (
    <Card className="bg-white text-gray-900 border border-gray-200 admin-dark:border-gray-700 admin-dark:bg-gray-900 admin-dark:text-gray-100">
      ...
      <CardContent className="">
        <div className="space-y-2 overflow-x-auto h-70 scrollbar-hide">
          {topSamples.map((p, i) => (
            <div
              key={p.id}
