...
import {
  ...
  ChevronLeft, ChevronRight, PackageOpen, X
} from 'lucide-react';
...
const useServiceData = () => {
  ...
  const isServiceProvided = useCallback((serviceId, titleVi) => {
...
const SkeletonLoader = () => (
  <div className="p-4 sm:p-6 bg-background dark:bg-slate-950 admin-dark:bg-slate-950 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 admin-dark:border-slate-800 max-w-4xl mx-auto animate-in fade-in duration-700">
    ...
    <p className="mt-4 text-center text-xs text-gray-500 admin-dark:text-gray-400">Đang tải dữ liệu...</p>
  </div>
);
...
const ErrorDisplay = ({ error }) => (
  <div className="flex flex-col items-center justify-center p-6 sm:p-8 bg-white rounded-xl shadow-lg border-2 border-red-300 max-w-md mx-auto my-10
    ...
    <h3 className="text-xl font-semibold text-gray-800 mb-2 admin-dark:text-white">Dữ liệu không sẵn sàng</h3>
    <p className="text-gray-600 mb-6 text-center text-sm admin-dark:text-gray-400">Không thể tải cấu hình dịch vụ. Vui lòng kiểm tra lại kết nối hoặc cài đặt.</p>
...
const EmptyServiceCard = () => (
  <div className="text-center py-8 px-4 flex flex-col items-center justify-center h-full bg-black/10 rounded-lg animate-in fade-in duration-500">
    ...
    <p className="text-base font-semibold text-white/80 mb-1">Chưa có hạng mục</p>
    <p className="text-sm text-white/60">Chưa có hạng mục nào được định nghĩa.</p>
  </div>
);


// --- 3. COMPONENT CARD DỊCH VỤ: Giữ nguyên (nhận prop index và currentIndex) ---
const ServiceCardMinimalist = ({ maxHeight, cardRef, service, uniqueTitles, isServiceProvided, stageColor, index, currentIndex }) => {
  const isCurrent = index === currentIndex;

  return (
    <div className="py-3 px-2 flex-shrink-0 xs:w-70 sm:w-90 md:w-100 snap-center transition-all duration-500 ease-out"
      data-index={index}
      ref={cardRef}>
      <div
        className={cn(
          ...
          isCurrent ? "scale-[1.03] shadow-2xl ring-4 ring-white/50" : "scale-100 opacity-80 hover:opacity-100",
      ...
      >
        <div className={cn("-z-1 absolute top-0 left-0 w-full h-[100%] ", stageColor.bg1Radius)}></div>
        <div className={cn("-z-1 absolute top-0 left-0 w-full h-[80%]  rounded-br-[80px]", stageColor.bg)}></div>
        <div className={cn("-z-1 absolute bottom-0 left-0 w-full h-[20%] ", stageColor.bgRadius)}></div>
        <div className={cn("-z-1 absolute bottom-0 left-0  w-full h-[20%] rounded-tl-[50px]", stageColor.bg1Radius)}></div>
                ...
                return (
                  <li
                    ...
                    className={cn(
                      ...
                      provided ? "text-white font-medium transform translate-x-0" : "text-white/60 opacity-80 transform translate-x-1"
                    )}
                  >
                    {provided ? (
        ...
        <button className="mt-6 w-full bg-white/90 font-bold py-3 rounded-3xl hover:bg-white transition-all duration-300 **shadow-lg hover:shadow-2xl hover:ring-4 hover:ring-white/80** will-change-transform">
          <span className={cn(stageColor.text, "text-base font-bold uppercase")}>Xem chi tiết</span>
        </button>
      </div>
    </div>
  );
...
const ServiceMinimalistView = () => {
  const { loading, error, uniqueServices, uniqueTitles, isServiceProvided } = useServiceData();
  ...
  const observeCards = useCallback(() => {
    ...
    observerRef.current = new IntersectionObserver(
      (entries) => {
        ...
        if (maxRatio > 0.5) { // Chỉ cập nhật nếu card chiếm hơn 50% tầm nhìn
          ...
          if (visibleEntries.length > 0) {
            ...
            const mostVisibleIndex = mostVisible.target?.getAttribute('data-index');
  ...
  const scrollToCard = useCallback((index) => {
    ...
    slider.children[index].scrollIntoView({ behavior: 'smooth', inline: 'center', block: 'nearest' });
  ...
  const stageColorMap = {
    1: { bg: "bg-gradient-to-b from-gray-500 to-gray-700", text: "text-gray-700", bgRadius: "bg-gray-700", bg1Radius: "bg-gray-500" },
    2: { bg: "bg-gradient-to-b from-green-500 to-green-700", text: "text-green-700", bgRadius: "bg-green-700", bg1Radius: "bg-green-500" },
    3: { bg: "bg-gradient-to-b from-blue-500 to-indigo-700", text: "text-indigo-700", bgRadius: "bg-indigo-700", bg1Radius: "bg-indigo-500" },
    4: { bg: "bg-gradient-to-b from-purple-500 to-purple-700", text: "text-purple-700", bgRadius: "bg-purple-700", bg1Radius: "bg-purple-500" },
    5: { bg: "bg-gradient-to-b from-pink-500 to-rose-600", text: "text-rose-600", bgRadius: "bg-rose-700", bg1Radius: "bg-rose-500" },
    6: { bg: "bg-gradient-to-b from-teal-500 to-teal-700", text: "text-teal-700", bgRadius: "bg-teal-700", bg1Radius: "bg-teal-500" },
  };
  const defaultColor = { bg: "bg-gradient-to-b from-cyan-500 to-teal-700", text: "text-cyan-700", bgRadius: "bg-cyan-700", bg1Radius: "bg-cyan-500" };

  return (
    <div className="p-4 bg-background dark:bg-slate-950 admin-dark:bg-slate-950 rounded-2xl shadow-xl max-w-4xl mx-auto relative group animate-in fade-in duration-500">

      <div role="region" aria-label="Carousel of configured services" className="relative">
        {/* Nút Previous */}
        <button
          onClick={handlePrev}
...
export default ServiceMinimalistView;